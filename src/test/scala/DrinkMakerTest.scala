...
class DrinkMakerTest extends FunSuite {
  test("Make Coffee") {
    val drink = DrinkMaker("Coffee", 0, 0.6)

    assert(drink.flavor == "Coffee")
  }

  test("Make Tea") {
    val drink = DrinkMaker("Tea", 0, 0.4)

    assert(drink.flavor == "Tea")
  }

  test("Make Chocolate") {
    val drink = DrinkMaker("Chocolate", 0, 0.5)
  ...
  test("Make drink with 1 sugar") {
    val drink = DrinkMaker("Chocolate", 1, 0.5)

    assert(drink.sugarCount == 1)
  }

  test("Make drink with 2 sugar") {
    val drink = DrinkMaker("Chocolate", 2, 0.5)
