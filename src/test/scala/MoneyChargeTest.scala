...
class MoneyChargeTest extends FunSuite {
  test("Make Coffee when correct amount is deposited") {
    val drink = DrinkMaker(DrinkOrder("Coffee", 1), 0.6)

    assert(drink != null)
  }

  test("Make Tea when correct amount is deposited") {
    val drink = DrinkMaker(DrinkOrder("Tea", 1), 0.4)

    assert(drink != null)
  }

  test("Make Chocolate when correct amount is deposited") {
    val drink = DrinkMaker(DrinkOrder("Tea", 1), 0.5)
  ...
  test("Make drink when bigger amount is deposited") {
    val drink = DrinkMaker(DrinkOrder("Coffee", 1), 0.7)

    assert(drink != null)
  }

  test("Reject to make drink when incorrect amount is deposited") {
    the[AmountNotSufficientException] thrownBy DrinkMaker(DrinkOrder("Coffee", 1), 0.4) should have message "Amount not sufficient: 0.4"
  }
}
